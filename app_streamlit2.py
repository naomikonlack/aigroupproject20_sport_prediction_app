...
predictions = [tree.predict(scaler.transform(input_df)) for tree in model.estimators_]
std_dev = np.std(predictions)

# Convert standard deviation to confidence level
max_sd = 10  # You might want to adjust this based on your domain knowledge
confidence_percentage = (1 - (std_dev / max_sd)) * 100
confidence_percentage = max(0, min(100, confidence_percentage))  # Clamp between 0 and 100
...
if st.button('Predict'):
    ...
    st.write(f"Standard Deviation of Predictions: {std_dev:.2f}")
    st.write(f"Confidence Level: {confidence_percentage:.2f}%")
    st.write("The lower the standard deviation, the higher the confidence in the prediction. A high confidence level means our model's individual trees mostly agreed on this prediction.")